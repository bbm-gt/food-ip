...
def test_semantic_requests_govern_natural_corrections_and_meaning_level_review() -> None:
    ...
    for stage in ("EXPLORE", "DEEPEN"):
        ...
        assert "无论A是否已在" in prompts[stage]
        assert "都必须只输出一条 CORRECT" in prompts[stage] or "都只输出一条 CORRECT" in prompts[stage]
        ...
        assert "不要改成 ADD" in prompts[stage]
        ...
        assert "只能用前述 CORRECT 表达" in prompts[stage]
