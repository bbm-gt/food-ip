...
SEMANTIC_STAGE_PROMPTS: dict[str, str] = {
    "EXPLORE": (
        ...
        "不得把被否定的A新增为事实，只输出一条针对B的事实变化，statement 始终写更正后的B。"
        "必须先按含义在 semantic_context.facts 中匹配A；若匹配，"
        ...
        "replaces_statement 必须为 null，不要填写老板原话中的A；此时也可以把B作为 ADD 输出。"
        "不要为A输出 ADD、REMOVE 或隐藏记录。转换层会清除匹配的旧项；均无匹配时直接新增B。"
        "只要 semantic_context.owner_message 明确否定A并确认B，且这属于餐厅或经营事实，本轮 new_facts"
        " 就必须包含这次更正；不得因A不在 facts、素材看似已足够或当前主要"
        "任务是找方向而省略。若无法可靠理解B，必须 ASK_OWNER 澄清，不能用方向结果忽略这次更正。"
        "statement 可以忠实整理老板明确表达的完整语义，不要求逐字复制 owner_quote，但不得补入老板"
        "没有表达的事实；例如‘汤不是当天才现熬，是前一晚开始准备，第二天用完’必须记入前一晚"
        "开始准备、第二天用完这一事实。"
    ...
    "DEEPEN": (
        ...
        "semantic_context.facts，都只输出一条针对B的事实变化，statement 写B。必须先按含义在 semantic_context."
        "facts 中匹配A；若匹配，使用 CORRECT，replaces_statement 必须逐字复制该有效事实的完整 statement。只有 facts 无匹配时，"
        "才按含义匹配 semantic_context.unconfirmed_inferences，并逐字复制该待确认项的完整 statement；两者"
        "均无匹配时，replaces_statement 必须为 null，不要填写老板原话中的A；此时也可以把B作为 ADD"
        " 输出。不要为A输出 ADD、REMOVE 或隐藏记录。转换层会清除匹配"
        "的旧项；均无匹配时直接新增B；"
        "只要 semantic_context.owner_message 明确否定A并确认B，且这属于餐厅或经营事实，本轮 new_facts"
        " 就必须包含这次更正；不得因A不在 facts、素材看似已足够或当前主要"
