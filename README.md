...
脚本主路径使用“规则选题 + DeepSeek 生成 + 程序质检”：规则负责事实与拍摄边界，AI 负责连贯表达和详细导演指导，默认返回三套方向不同的方案。生成通过程序硬校验后，会自动附加一轮 AI 编导 9 维审稿与程序化低分判定；自动局部修稿尚未上线。旧版模板接口继续保留用于兼容已有项目；视频成片润色仍为后续能力。
