...
- 大模型可以把老板本轮明确表达的事实忠实整理成完整、可复用的 `statement`；整理结果不要求逐字包含于 `owner_quote`。`owner_quote` 仍须是老板本轮消息中的连续原文并作为 Evidence，语义整理不得引入老板未表达、Knowledge 提供或 AI 推测的餐厅事实。
- 应用层不得仅因 `statement` 不是老板原话的逐字片段而把它降级为未确认推断；没有匹配旧事实时，语义上的新增或更正都直接加入当前有效事实。
