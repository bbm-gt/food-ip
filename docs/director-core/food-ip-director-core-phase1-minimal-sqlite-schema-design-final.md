...
- 正常更新必须使用 `WHERE session_id = ? AND state_version = ?` 的乐观条件，且新版本严格等于旧版本加一；受影响行数不是 1 即整轮失败。该 CAS 的 OLD version 只属于正常业务更新语义。
...
- READY 后禁止任何产生新版本的业务更新。确定性修复不是新创作：如需从 Turn snapshot 恢复损坏投影，只允许受控维护路径写回权威最大 Turn 的版本、Stage、latest Turn 和经重新计算一致的哈希，不能借恢复名义创造更高逻辑版本；READY Session 也只允许这一条严格的确定性修复路径。
- Working State 正常业务更新与确定性恢复是两个不同路径。恢复不信任待修复 Working State 的 OLD version、Stage、latest Turn、JSON 或 hash；恢复版本来自当前 Session 最大成功 Turn，而不是 OLD State。恢复既可修复现有行，也可重建丢失行，但只能选择当前 Session 最大 `post_state_version` 的完整成功 Turn snapshot；不得使用较早 snapshot 降级，也不要求较早 snapshot 全部健康。Recovery Turn 的 trace 只读取同 Session 直接前一 Turn 的 `target_stage` 作为 authoritative pre-stage（v0 固定为 EXPLORE），不重放或递归验证完整历史。READY 恢复还必须验证 ReadyContent v1（版本、UUID、Session/Turn 归属、UTC 毫秒时间、canonical JSON、严格模型、Session.ready_at、首次响应 ID 与 snapshot 最终 `draft.content`）完整闭合。
- 哈希定义唯一且固定为：
